...
```{r echo=FALSE}
dcast(ddply(g, .(type, status), summarize, cnt=length(start)), 
...
```{r echo=FALSE}
ddply(subset(g, status == "Approved" & type == "New"), .(state), summarize,
      meanDays=as.numeric(mean(approv-start, na.rm=TRUE)),
      meanMonths=as.numeric(mean(approv-start, na.rm=TRUE))/30,
      minDays=as.numeric(min(approv-start, na.rm=TRUE)),
      maxDays=as.numeric(max(approv-start, na.rm=TRUE)),
...
```{r echo=FALSE}
ddply(subset(g, status == "Approved" & type == "Supplemental"), .(state), summarize,
      meanDays=as.numeric(mean(approv-start, na.rm=TRUE)),
      meanMonths=as.numeric(mean(approv-start, na.rm=TRUE))/30,
      minDays=as.numeric(min(approv-start, na.rm=TRUE)),
      maxDays=as.numeric(max(approv-start, na.rm=TRUE)),
